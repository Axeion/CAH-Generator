...
$(document).ready(function () {
    ...
    /([^_\\])_([^_])|([^_\\])_([^_]?)$|^([^_\\]?)_([^_])/g,
    '$1$3$5___________________$2$4$6').replace(/\\_/g, '_'));
        ...
        if (batch_count < 201 && text.val() !== "") {
            ...
            if (text.val() == "") {
                $('<div inert class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert">&times;</button><strong>Oh Snap!</strong> You didn\'t enter in any cards to generate, stupid.</div>')
                    .prependTo(form);
            }
            // Too many cards were entered
            if (batch_count >= 201) {
                $('<div inert class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert">&times;</button><strong>Oh Snap!</strong> The generator can\'t make more than 30 cards at once. Learn to read.</div>')
                    .prependTo(form);
            }
        }
